...
$id = $_GET["id"] ?? "";
$response = $client->getMetadata($id);
            ...
            <div class="box-body">
                ...
                <form id="sendReferenceForm">
                    <div class="form-group">
                        ...
                        <select type="text" class="form-control" id="page_id" name="page_id">
                            ...
                            <?php foreach ($pages as $row): ?>
                                <option <?php if ($row['id'] == $meta['page_id'])
                                    echo "selected"; ?> value="<?= $row['id']; ?>">
                                    <?= $row['name']; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" value="<?= $meta['title']; ?>" class="form-control" id="title" name="title"
...
<script>
    $(function () {
        ...
        $('#sendReferenceForm').on('submit', async function (e) {
            ...
            try {
                const response = await fetch('bridge.php?mode=update_metadata&id=<?= $id; ?>', {
                    method: 'POST',
                ...
                if (response.ok) {
                    $('#sendRefSuccess').html('Metadata updated successfully').show();
